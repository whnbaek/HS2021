...
FPGA::FPGA(off_t data_addr, off_t output_addr, int m_size, int v_size)
{
  ...
  qout_ = new int[m_size_]; // quantized
  ...
  qdata_M = static_cast<char *>(mmap(NULL, m1_size_ * sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, fd_, data_addr));
  out_M = new float[m1_size_];
...
char *FPGA::qmatrix_Offset(void)
{
  return qdata_M + m1_size_ + m2_size_;
}

int *FPGA::qmatrix_Out(void)
{
  return (int *)qdata_M;
}
...
const float *__attribute__((optimize("O0"))) FPGA::blockMM(Compute* comp)
{
  num_block_call_ += 1;
  ...
  if(comp->quantized)
  {
    ...
    char *qoffset = this->qmatrix_Offset();
    qoffset[0] = weight_offset;
    qoffset[1] = act_offset;
    ...
    *output_ = 0x5555;
    ...
    int *qout_M_ = this->qmatrix_Out();

    dequantize(qout_M_, out_M, m1_size_, act_scale * weight_scale);
  }
  else{
    for(int i = 0; i < v_size_; ++i)
    {
      for(int j = 0; j < v_size_; ++j){    
        ...
        for(int k = 0; k < v_size_; ++k){
          out_M[v_size_*i+j] += m1[v_size_*k+i] * m2[v_size_*k + j];
        }
      }
    }
  }
  ...
  for(int i = 0; i < num_output; i += v_size_)
  {
    ...
    {			
      for(int k = 0; k < num_matrix2; k += v_size_)
      {
        ...
        // 1) Assign a m1 with transpose
        for (int l = 0; l < block_row; l++) {
          for (int m = 0; m < block_col_1; m++)
            m1[l + m * v_size_] = weight_mat[(i + l) * num_input + (j + m)];
