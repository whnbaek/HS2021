...
FPGA::FPGA(off_t data_addr, off_t output_addr, int m_size, int v_size)
{
  ...
  m1_size_ = v_size * v_size / 2;
  m2_size_ = v_size * v_size;

  data_size_ = (m_size_+1)*v_size_; // fpga bram data size
  data_size_M = (m1_size_ + m2_size_)*sizeof(float)+8*sizeof(char);
...
char *FPGA::matrix_mask(void)
{
  return (char *)(data_M + m1_size_ + m2_size_);
}
  ...
  for(int i = 0; i < num_output; i += m_size_)
  {
    ...
    {			
      ...
      for (int y = 0; y < block_row; y++) {
        for (int x = 0; x < block_col; x++)
          mat[y * v_size_ + x] = large_mat[(i + y) * num_input + j + x];
        for (int x = block_col; x < v_size_; x++)
          mat[y * v_size_ + x] = 0;
      }
      for (int y = block_row; y < m_size_; y++)
...
void FPGA::largeMM(const float* weight_mat, const float* input_mat, float* output, int num_input, int num_output, int num_matrix2)
{
  ...
  char *mask = this->matrix_mask();

  std::pair<float, int> v[8];
  int w[4];
      ...
      for(int k = 0; k < num_matrix2; k += v_size_)
      {
        ...
        for (int l = 0; l < block_row; l++) {
          for (int m = 0; m < block_col_1; m++)
            v[m] = std::make_pair(std::abs(weight_mat[(i + l) * num_input + (j + m)]), m);
          for (int m = block_col_1; m < v_size_; m++)
            v[m] = std::make_pair(0.0, m);
          mask[l] = 0;        

          std::sort(v, v + 4); 
          mask[l] |= (1 << v[2].second);
          mask[l] |= (1 << v[3].second);
          m1[l * v_size_ / 2] = weight_mat[(i + l) * num_input + (j + min(v[2].second, v[3].second))];
          m1[l * v_size_ / 2 + 1] = weight_mat[(i + l) * num_input + (j + max(v[2].second, v[3].second))];

          std::sort(v + 4, v + 8); 
          mask[l] |= (1 << v[6].second);
          mask[l] |= (1 << v[7].second);
          m1[l * v_size_ / 2 + 2] = weight_mat[(i + l) * num_input + (j + min(v[6].second, v[7].second))];
          m1[l * v_size_ / 2 + 3] = weight_mat[(i + l) * num_input + (j + max(v[6].second, v[7].second))];
        }
        for (int l = block_row; l < v_size_; l++) {
          mask[l] = 85;
          for (int m = 0; m < v_size_ / 2; m++)
            m1[l * v_size_ / 2 + m] = 0;
        }
